...
class ActivityProgressRepository @Inject constructor(private val progressDao: ActivityProgressDao) {
    ...
    private fun ActivityProgressEntity.toModel() = ActivityProgress(activityId = activityId, periodKey = periodKey, currentAmount = currentAmount, currentCount = currentCount, currentCashback = currentCashback, todayCashback = todayCashback, isAchieved = isAchieved, continuousDone = continuousDone)
    private fun ActivityProgress.toEntity() = ActivityProgressEntity(activityId = activityId, periodKey = periodKey, currentAmount = currentAmount, currentCount = currentCount, currentCashback = currentCashback, todayCashback = todayCashback, isAchieved = isAchieved, continuousDone = continuousDone)
}
